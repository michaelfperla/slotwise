...
export default function EditServicePage() {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    fetch(`/api/v1/services/${serviceId}`, { // Business Service GET service by ID
      headers: { 'Authorization': `Bearer ${token}` },
    })
      .then(async res => {
        if (!res.ok) {
          const errorData = await res.json().catch(() => ({ message: `Failed to fetch service: ${res.statusText}` }));
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/v1/services/${serviceId}`, { // Business Service PUT endpoint
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ message: `Failed to update service: ${response.statusText}` }));
        throw new Error(errorData.message);
      }
  ...
  if (error && !formData.name) return <p style={{ color: 'red' }}>Error loading service: {error}</p>; // Show critical error if form can't load
    ...
    <div style={{ maxWidth: '600px', margin: 'auto', padding: '20px' }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
        ...
        <div>
          ...
          <input type="number" name="duration" id="duration" value={formData.duration || 0} onChange={handleChange} required min="1" style={inputStyle} />
        </div>
        <div>
          <label htmlFor="price" style={labelStyle}>Price:</label>
          <input type="number" name="price" id="price" value={formData.price || 0} onChange={handleChange} required min="0" step="0.01" style={inputStyle} />
        </div>
        <div>
          <label htmlFor="currency" style={labelStyle}>Currency (e.g., USD):</label>
          <input type="text" name="currency" id="currency" value={formData.currency || 'USD'} onChange={handleChange} maxLength={3} style={inputStyle} />
        </div>
        <div>
          <label htmlFor="isActive" style={labelStyle}>
            <input type="checkbox" name="isActive" id="isActive" checked={formData.isActive === undefined ? true : formData.isActive} onChange={handleChange} style={{ marginRight: '5px' }} />
            Service is Active
          </label>
        </div>
...
const inputStyle: React.CSSProperties = { width: '100%', padding: '10px', border: '1px solid #ddd', borderRadius: '4px', boxSizing: 'border-box' };
const buttonStyle: React.CSSProperties = { padding: '10px 20px', border: 'none', borderRadius: '4px', cursor: 'pointer', backgroundColor: '#0070f3', color: 'white', fontWeight: 'bold' };
