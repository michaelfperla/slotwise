...
interface TimeSlot {
  // Matches what Scheduling Service returns
...
export default function BookServicePage() {
  ...
  useEffect(() => {
    ...
    const fetchServiceDetails = async () => {
      try {
        ...
        if (result.success && result.data.businessId === businessId) {
          // Ensure service belongs to the business in query
          setServiceDetails(result.data);
        } else {
          throw new Error(
            result.message || 'Service not found or does not belong to this business.'
          );
        }
      } catch (e: unknown) {
      ...
      .then(async res => {
        if (!res.ok) {
          const errorData = await res
            .json()
            .catch(() => ({ message: `Failed to fetch slots: ${res.statusText}` }));
  ...
  const handleProceedToConfirmation = () => {
    ...
    router.push(
      `/booking/confirm?serviceId=${serviceId}&serviceName=${encodeURIComponent(serviceDetails.name)}&businessId=${businessId}&startTime=${encodeURIComponent(selectedSlot.startTime)}&endTime=${encodeURIComponent(selectedSlot.endTime)}&price=${serviceDetails.price}`
    );
  };
  ...
  if (!businessId && !error)
    return (
      <p style={{ color: 'red' }}>
        Business ID is required in URL query parameters (e.g., ?businessId=...).
      </p>
    );
  if (!serviceDetails && !error)
    return (
      <p style={{ color: 'red' }}>
        Service details could not be loaded. Ensure the service ID and business ID are correct.
      </p>
    );
  ...
  return (
    <div style={{ maxWidth: '700px', margin: 'auto', padding: '20px' }}>
      ...
      {serviceDetails && (
        <p>
          Duration: {serviceDetails.duration} minutes. Price: ${serviceDetails.price}
        </p>
      )}
      {error && <p style={{ color: 'red' }}>Error: {error}</p>}

      <div>
        <label htmlFor="booking-date" style={{ marginRight: '10px' }}>
          Select Date:
        </label>
        <input
          type="date"
        ...
        {availableSlots.map((slot, index) => (
          <button
            ...
            style={{
              padding: '10px 15px',
              border:
                selectedSlot?.startTime === slot.startTime ? '2px solid #0070f3' : '1px solid #ccc',
              borderRadius: '5px',
              cursor: 'pointer',
