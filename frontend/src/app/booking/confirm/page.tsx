    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response
          .json()
          .catch(() => ({ message: `Booking failed: ${response.statusText}` }));
  ...
  if (error) {
    return (
      <div style={{ padding: '20px', color: 'red' }}>
        Error: {error} <button onClick={() => router.back()}>Go Back</button>
      </div>
    );
  }
  ...
  if (bookingConfirmed && confirmedBookingDetails) {
    return (
      <div style={{ maxWidth: '600px', margin: 'auto', padding: '20px', textAlign: 'center' }}>
        ...
        <p>
          Your booking (ID: {confirmedBookingDetails.bookingId.substring(0, 8)}...) is{' '}
    ...
    <div style={{ maxWidth: '600px', margin: 'auto', padding: '20px' }}>
      ...
      {bookingDetails && (
        <div style={{ border: '1px solid #ccc', padding: '20px', borderRadius: '5px' }}>
          <p>
            <strong>Service:</strong> {decodeURIComponent(bookingDetails.serviceName)}
          </p>
          <p>
            <strong>Date:</strong> {new Date(bookingDetails.startTime).toLocaleDateString()}
          </p>
          <p>
            <strong>Time:</strong>
