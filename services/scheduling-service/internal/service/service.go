...
func (s *BookingService) UpdateBookingStatus(ctx context.Context, bookingID string, newStatus models.BookingStatus) (*models.Booking, error) { // Add import for client "github.com/slotwise-app/services/scheduling-service/internal/client"
	...
	if booking.ServiceID != "" {
		...
		if errService == nil && serviceDef != nil {
			...
			s.logger.Warnw("Could not fetch service details for notification data", "bookingId", bookingID, "serviceId", booking.ServiceID, "error", errService)
	...
	if s.notificationClient != nil {
		...
		case models.BookingStatusConfirmed:
			...
			if errPub := s.eventPublisher.Publish(events.SlotReservedEvent, slotReservedPayload); errPub != nil {
				s.logger.Errorw("Failed to publish slot.reserved event", "bookingId", booking.ID, "error", errPub)
			} else {
				s.logger.Infow("Published slot.reserved event", "bookingId", booking.ID)
			}
			...
			if err != nil {
				s.logger.Errorw("Failed to send booking confirmation to customer", "bookingId", booking.ID, "error", err)
			...
			if err != nil {
				s.logger.Errorw("Failed to send booking confirmation to business", "bookingId", booking.ID, "error", err)
			}
			...
			if reminderTime.After(time.Now()) {
				...
				if err != nil {
					s.logger.Errorw("Failed to schedule booking reminder", "bookingId", booking.ID, "error", err)
				}
			} else {
				s.logger.Infow("Booking reminder time is in the past, not scheduling.", "bookingId", booking.ID, "reminderTime", reminderTime)
			}
		...
		case models.BookingStatusCancelled:
			...
			if err != nil {
				s.logger.Errorw("Failed to send booking cancellation to customer", "bookingId", booking.ID, "error", err)
			}
			// Optionally, notify business about cancellation

		default:
			s.logger.Infow("No specific NATS event or notification for status update", "bookingId", booking.ID, "newStatus", newStatus)
		}
	}
	...
	if eventSubject != "" {
		if errPub := s.eventPublisher.Publish(eventSubject, eventPayload); errPub != nil {
			s.logger.Errorw("Failed to publish booking status event", "subject", eventSubject, "bookingId", booking.ID, "error", errPub)
		} else {
			s.logger.Infow("Published booking status event", "subject", eventSubject, "bookingId", booking.ID)
		}
	}
