...
export async function handleBookingConfirmed(payload: BookingConfirmedPayload): Promise<void> {
  ...
  const bookingTime = new Date(payload.startTime).toLocaleTimeString(undefined, {
    hour: '2-digit',
    minute: '2-digit',
    hour12: true,
  });
  ...
  try {
    ...
    logger.error('Failed to send booking confirmation to customer', {
      bookingId: payload.bookingId,
      error,
    });
  }
  ...
  try {
    ...
    logger.info('New booking notification email sent to business', {
      bookingId: payload.bookingId,
      businessOwnerEmail: payload.business.ownerEmail,
    });
  } catch (error) {
    logger.error('Failed to send new booking notification to business', {
      bookingId: payload.bookingId,
      error,
    });
  }
  ...
  logger.info(
    `Reminder scheduling (MVP): Logged for booking ${payload.bookingId} at ${reminderTime.toISOString()}`
  );
}
...
export async function handleBookingCancelled(payload: BookingCancelledPayload): Promise<void> {
  ...
  try {
    ...
    logger.error('Failed to send booking cancellation to customer', {
      bookingId: payload.bookingId,
      error,
    });
  }
  ...
  try {
    ...
    logger.error('Failed to send booking cancellation notification to business', {
      bookingId: payload.bookingId,
      error,
    });
  }
