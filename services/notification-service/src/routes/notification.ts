...
export async function notificationRoutes(fastify: FastifyInstance) {
  ...
  }, async (request: FastifyRequest<{ Body: z.infer<typeof sendNotificationSchema> }>, reply: FastifyReply) => {
    try {
  ...
  }, async (request: FastifyRequest<{ Querystring: z.infer<typeof notificationQuerySchema> }>, reply: FastifyReply) => {
    try {
      const userId = request.user?.id;
      if (!userId) {
        return reply.code(401).send({
  ...
  fastify.get('/:id', async (request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    try {
      ...
      const notification = await notificationService.getNotificationById(request.params.id, userId);
